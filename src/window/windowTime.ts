/** Legacy time widget display. Keeping for future reference.
 * See ./statistics/timeSpent.ts for the current way it's implemented. */
...
export function getTimeWidget(): WidgetCreator<FlexiblePosition> {
  return groupbox({
    text: "Time Spent In",
    content: [getGameTimeWidget(), getParkTimeWidget()],
  });
}

function getGameTimeWidget(): WidgetCreator<
  FlexiblePosition,
  Parsed<FlexiblePosition>
> {
  let gameTime = store(formatTime(time.timeData.gameTime.get()));
  time.timeData.gameTime.subscribe((newTime) =>
    gameTime.set(formatTime(newTime))
  );

  return horizontal([label({ text: "OpenRCT2 -" }), label({ text: gameTime })]);
}

function getParkTimeWidget(): WidgetCreator<
  FlexiblePosition,
  Parsed<FlexiblePosition>
> {
  ...
  time.timeData.parkTime.subscribe((newTime) =>
    parkTime.set(formatTime(newTime))
  );

  let isVisible = store(getElementVisibility(events.isInPark.get()));
  events.isInPark.subscribe((inPark) =>
    isVisible.set(getElementVisibility(inPark))
  );
