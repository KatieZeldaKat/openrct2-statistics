/**
 * Legacy statistic code that inspired the current object-oriented approach.
 * Leaving for visibility and potential future reference.
 */
...
export const timeData = {
  ...
  parkTime: store(
    context.mode == "normal"
      ? context.getParkStorage().get(PARK_TIME_KEY, 0)
      : 0
  ),
};
...
export function initialize() {
  timeData.gameTime.subscribe((newTime) =>
    context.sharedStorage.set(GAME_TIME_KEY, newTime)
  );
  timeData.parkTime.subscribe((newTime) =>
    context.getParkStorage().set(PARK_TIME_KEY, newTime)
  );
